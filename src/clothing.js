export async function clothingSuggestions(req) {
  console.log('Clothing suggestions request:', req.payload);

  const { temperature, description, rain, snow, season, region } = req.payload;
  ...
  try {
    ...
    throw new Error('Failed to generate clothing suggestions');
  }
}
...
function generateClothingRecommendations(temperature, description, rain, snow, season, region) {
  ...
  if (rain || lowerDescription.includes('rain')) {
    ...
    // Parse rain data if provided as JSON string
    if (rain && typeof rain === 'string') {
      try {
  ...
  if (snow || lowerDescription.includes('snow')) {
    ...
    // Parse snow data if provided as JSON string
    if (snow && typeof snow === 'string') {
      try {
